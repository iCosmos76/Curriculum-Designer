...
namespace Kernel_of_curriculum {
    ...
    public class SqliteDataAccessDisp {
        ...
        public static List<Discipline_Conn_Data_Base> LoadDisp() {
            ...
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString())) {
                var output = cnn.Query<Discipline_Conn_Data_Base>(
                    "select idDisp, NameD, SokrNameD, Blok, NameK, TheoryRab, FlagPromAttest, PromAttest, FlagKurs, Kurs " +
        ...
        public static void AddDisp(string nameD,string sokrNameD,
            ...
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString())) {
                cnn.Execute("insert into Discipline (NameD, SokrNameD, Blok, idK, " +
        ...
        public static void UpdateSorkNameD(int IdDisp,string SorkNameD) {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString())) {
                cnn.Execute("update Discipline " +
                    $"set SorkNameD = '{SorkNameD}' " +
                    $"where idDisp = {IdDisp}");
            }
        }

        public static void UpdateSorkBlok(int IdDisp, int Blok) {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString())) {
                cnn.Execute("update Discipline " +
                    $"set Blok = '{Blok}' " +
                    $"where idDisp = {IdDisp}");
